...
def intrinsics_to_matrix(intrinsics):
    ...
    if model == 'SIMPLE_PINHOLE':
        ...
        focal_length, cx, cy, _ = params  # Last part is radial distortion coefficient
        fx = fy = float(focal_length)
...
def resize_with_padding_torch(img_tensor, target_size, black=False, return_unpadded=False):
    """
    Resize a PyTorch tensor image to the target size, adding padding to the shorter side to maintain aspect ratio.
    # assuming 0,1 not 0,255!!
    :param img_tensor: Image tensor of shape (C, H, W)
    :param target_size: Int specifying the target size for both width and height
    :param black: Bool, True for black padding, False for white padding
    """
    target_size = (target_size, target_size)

    # Calculate the ratio of the target size and the original size
    _, height, width = img_tensor.shape
    width_ratio = target_size[0] / height
    height_ratio = target_size[1] / width
    ...
    # Resize the image
    resized_img = torchvision.transforms.functional.resize(img_tensor, new_size, antialias=True)

    if return_unpadded:
        return resized_img

    # Calculate padding
    padding_left = (target_size[1] - new_size[1]) // 2
    padding_top = (target_size[0] - new_size[0]) // 2
    padding_right = target_size[1] - new_size[1] - padding_left
    padding_bottom = target_size[0] - new_size[0] - padding_top

    # Apply padding
    pad_value = 0 if black else 1
    padded_img = torchvision.transforms.functional.pad(resized_img, (padding_left, padding_top, padding_right, padding_bottom), fill=pad_value)
...
def resize_with_padding_np(img_path, target_size, black=False, interpolation='shrink'):
    target_size = (target_size, target_size)
    # Read the image
    if isinstance(img_path,str):
        img = cv2.imread(img_path)
    ...
    # Resize the image
    interpolation = cv2.INTER_AREA if interpolation=='shrink' else cv2.INTER_LANCZOS4
    resized_img = cv2.resize(img, (new_size[0], new_size[1]), interpolation=interpolation) # takes W, H

    # Create the background, use float not int
    bg_color = (0.,) if black else (255.,)
    new_img = np.full((target_size[1], target_size[0], img.shape[2]), bg_color*img.shape[2])

    # Calculate the paste position
    paste_position = ((target_size[0] - new_size[0]) // 2, (target_size[1] - new_size[1]) // 2)

    # Paste the resized image onto the background
    new_img[paste_position[1]:paste_position[1]+new_size[1], paste_position[0]:paste_position[0]+new_size[0]] = resized_img

    return new_img

def add_padding_without_resizing(img_path, target_size, black=True):
    target_size = (target_size, target_size)
    # Read the image
    if isinstance(img_path,str):
        img = cv2.imread(img_path)
    ...
    # Paste the resized image onto the background
    new_img[paste_position[1]:paste_position[1]+new_size[1], paste_position[0]:paste_position[0]+new_size[0]] = img
