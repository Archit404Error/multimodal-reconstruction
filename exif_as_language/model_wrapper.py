...
class EXIFNet(torch.nn.Module):
    def __init__(self, model, avg_word_embs=False, transformer="BERT", visual="RN50", resolution=124, pretrained=True):
        ...
        if transformer == 'BERT':
            ...
            if pretrained: self.model.transformer = AlbertModel.from_pretrained("albert-large-v2")
            else:
...
def load_wrapper_model(device, state_dict_path=None, split_gpus=False, model_name="ViT-B/32", input_resolution=124, transformer="ALBERT", visual="RN50", pretrained=True):
